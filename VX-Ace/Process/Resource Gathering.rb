# ╔═══════════════════════════════════════════════╦════════════════════╗
# ║ Title: Resource Gathering                     ║  Version: 1.00     ║
# ║ Author: Roninator2                            ║                    ║
# ╠═══════════════════════════════════════════════╬════════════════════╣
...
$imported[:r2_ghm] = 1.00          # Gather Harvest Mine
...
module DataManager
  ...
  #--------------------------------------------------------------------------
  # new method: load_notetags_gather
  #--------------------------------------------------------------------------
...
class Game_Interpreter
  ...
  def gather_materials(*data)
    ...
    ability = actor.gather_skills[gather_skill][0]
    itsz = data.size - 3
    ...
    if gather_multi.downcase == "single"
      ...
      chance = $data_items[item].gather_chance
      bonus = $data_items[item].gather_bonus
      add = $data_items[item].gather_add.to_i
      num = rand(100)
      total = ability + chance + num - 100
      if total > 0
        amt = 1
        if bonus != 0
          bn = (one / bonus).to_i
      ...
      if guarantee == true
        ...
        $game_message.add(sprintf(Vocab::ObtainItem, $data_items[item].name))
        ...
        if amt > 0
          ...
          actor.gather_skills[gather_skill] += 1
        end
      end
    else
      success = false
      gather_items.each do |item|
        ...
        chance = $data_items[item].gather_chance
        bonus = $data_items[item].gather_bonus
        add = $data_items[item].gather_add
        num = rand(100)
        total = ability + chance + num - 100
        if total > 0
          amt = 1
    ...
    if actor.gather_skills[gather_skill] > actor.gather_max[gather_skill]
      actor.gather_skills[gather_skill] = actor.gather_max[gather_skill]
    end
  end
  ...
  def load_notetags_actor_gather(actor_id)
    ...
    $data_actors[actor_id].note.split(/[\r\n]+/).each { |line|
      ...
      when R2_GATHERING::GATHER_SKILL
        @gather_skills[$1.to_sym] = $2.to_i
      when R2_GATHERING::GATHER_MAX
        @gather_max[$1.to_sym] = $2.to_i
      end
    }
  end
end
